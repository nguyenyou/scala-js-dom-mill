...
/** The Transferable interface represents an object that can be transfered
  ...
  *
  * This is an abstract interface and there isn't any object of this type. It
  * also doesn't define any method or property: it is merely a tag indicating
  * objects that can be used in specific conditions, like to be transfered to a
  * Worker using the Worker.postMessage() method.
  */
...
/** defined at
  * [[https://fetch.spec.whatwg.org/#request-class ¶6.3 Request class]] of
  * whatwg Fetch spec
  */
type RequestInfo = String | Request

/** defined at [[https://fetch.spec.whatwg.org/#headersinit ¶6.1 Header Class]]
  * in whatwg Fetch spec todo: it should be OpenEndedDictionary[ByteString]
  */
type HeadersInit =
  Headers | Sequence[Sequence[ByteString]] | OpenEndedDictionary[ByteString]

/** This type should capture strings consisting only of ASCII chars todo: is
  * there a way to capture this type?
  */
type ByteString = String

/** defined at [[https://fetch.spec.whatwg.org/#bodyinit-unions]] in whatwg
  * Fetch spec
  */
type BodyInit =
  Blob | BufferSource | FormData | String | ReadableStream[Uint8Array] |
    URLSearchParams

/** WebIDL sequence<T> is js.Array[T] | JSIterable[T]. However @mseddon knows at
  * least Blink's IDL compiler treats these as simply js.Array[T] for now. We
  * keep this type as a reminder to check in more detail
  */
type Sequence[T] = js.Array[T]

/** see [[https://fetch.spec.whatwg.org/#headers-class ¶6.1 Headers class]] in
  * whatwg Fetch spec. also see:
  * [[https://github.com/whatwg/fetch/issues/164 issue 164]] in Fetch API git
  * repo, as this is not clearly defined
  */
...
@js.native
@JSGlobal("fetch")
def fetch(info: RequestInfo, init: RequestInit = null): js.Promise[Response] =
  js.native
...
/** A valid key path can include one of the following: an empty string, a
  * JavaScript identifier, or multiple JavaScript identifiers separated by
  * periods or an array containing any of those. It cannot include spaces.
  *
  * @see
...
/** According to
  * [[http://www.w3.org/TR/WebCryptoAPI/#algorithm-dictionary ¶11 Algorithm Identifier]]
  * of the WebCryptoAPI an AlgorithmIdentifier is an `object or DOMString`. We
  * make this more precise here and specify an Algorithm. note: it may be that
  * we can do only with KeyAlgorithmIdentifier and HashAlgorithmIdentifier
  */
...
/** According to
  * [[http://www.w3.org/TR/WebCryptoAPI/#algorithm-dictionary ¶11 Algorithm Identifier]]
  * of the WebCryptoAPI an AlgorithmIdentifier is an `object or DOMString`. We
  * make this more precise here and distinguish the non overlapping classes of
...
/** According to
  * [[http://www.w3.org/TR/WebCryptoAPI/#algorithm-dictionary ¶11 Algorithm Identifier]]
  * a HashAlgorithmIdentifier is an AlgorithmIdentifier. Here we distinguish
  * between Hash and Key Algorithm Identifiers. At the JS layer these have the
  * same structure.
  */
...
/** This is the same as `crypto` in JS.
  *
  * To maintain backwards-source-compatibility with scala-js-dom v1, `crypto` in
  * scala-js-dom is retained as a package object.
  */
@js.native
@JSGlobal("crypto")
